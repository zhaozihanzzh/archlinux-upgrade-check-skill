...
description: >
  ...
  Pacman and Package Upgrade Issues forum before running pacman -Syu, cross-referencing
  the user's pending package list to find anything needing manual intervention.
  Use this skill whenever the user is about to upgrade/update Arch Linux or run pacman -Syu
  and wants to know if anything needs manual handling first, even if they don't explicitly
  say check. Triggers: pre-upgrade check, before upgrade, pacman -Syu, Arch upgrade/update,
  manual intervention, upgrade safety. You must first read this skill's SKILL.md (the full
  instructions are not auto-loaded, only this description is) then follow it to run
  scripts/arch_upgrade_check.py with --report-dir. Do not curl or scrape archlinux.org
  yourself - the bundled script already fetches, parses, and cross-references the pending
  package list and is faster and more accurate than browsing by hand.
---
